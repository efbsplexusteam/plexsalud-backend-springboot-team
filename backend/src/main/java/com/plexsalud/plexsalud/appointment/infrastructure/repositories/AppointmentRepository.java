...
public interface AppointmentRepository extends CrudRepository<Appointment, Long> {
    @Query("SELECT a.id, a.date, a.status, p.fullName, p.age, p.gender, p.email FROM Appointment a JOIN a.patient p WHERE a.doctor = :doctor ORDER BY a.id")
    List<DoctorAppointmentResponse> findAllAppointmentsByDoctor(Doctor doctor);

    @Query("SELECT a.id, a.date, a.status, d.fullName, d.email, d.specialty FROM Appointment a JOIN a.doctor d WHERE a.patient = :patient ORDER BY a.id")
